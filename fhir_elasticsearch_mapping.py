...
async def get_searchable_resources(table_data):
    ...
    with_dot_as_or_is = re.compile(r"(\.as\([a-z]+\)$)|(\.is\([a-z]+\)$)", re.I)

    for group in table_data.keys():
        for row in table_data.get(group):
            if row[0] in searchable_data.keys():
                if len(row) == 4 and len(searchable_data[row[0]]) == 2:
                    searchable_data[row[0]][1].update(row[3])
                elif len(row) == 4 and len(searchable_data[row[0]]) == 1:
                    searchable_data[row[0]].insert(1, set(row[3]))
            else:
...
async def get_resources_schema(target_dir, offline):
    ...
    for root, dirs, files in os.walk(target_dir):
        ...
        for filename in files:
            ...
            resources_files[filename.split(".")[0]] = os.path.join(root, filename)
...
async def create_basic_datatype_map(datatype, **kw):
    ...
    if datatype == "boolean":
        ...
        if kw.get("pattern", None) in (
            mappings.date_pattern,
            mappings.datetime_pattern,
        ) or "Date/Time" in kw.get("description", ""):
...
async def main(destination_dir, output_stream, offline, verbosity_level):
    ...
    mappings_container["Resource"].update({"resourceType": mappings.Token})
    base_mapping = mappings_container["Resource"]

    for resource, paths in searchable_resources.items():
        await add_mapping(resource, schema_files[resource], paths, mappings_container)
    ...
    sys.stdout.write("Mapping files are generated at {0}\n".format(destination_dir))
    return 0
