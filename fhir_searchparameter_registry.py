...
async def get_parsed_content(offline, resource_url=None):
    ...
    if offline:
        ...
        if not cached_file.exists():
            sys.stderr.write("No offline file found! at {0!s}".format(cached_file))
...
async def main(destination_dir, output_stream, offline, verbosity_level):
    ...
    searchable_data["meta"] = dict(lastUpdated=None, versionId=None)
    ...
    for group in table_data.keys():
        for row in table_data.get(group):
            if row[0] in searchable_data["searchable"].keys():
                if len(row) == 4 and len(searchable_data["searchable"][row[0]]) == 2:
                    searchable_data["searchable"][row[0]][1].update(row[3])
                elif len(row) == 4 and len(searchable_data["searchable"][row[0]]) == 1:
                    searchable_data["searchable"][row[0]].insert(1, set(row[3]))
            else:
